package oauth2

import (
	"bytes"
	"compress/gzip"
	"fmt"
	"io"
	"strings"
)

func bindata_read(data []byte, name string) ([]byte, error) {
	gz, err := gzip.NewReader(bytes.NewBuffer(data))
	if err != nil {
		return nil, fmt.Errorf("Read %q: %v", name, err)
	}

	var buf bytes.Buffer
	_, err = io.Copy(&buf, gz)
	gz.Close()

	if err != nil {
		return nil, fmt.Errorf("Read %q: %v", name, err)
	}

	return buf.Bytes(), nil
}

var _migrations_sql_cockroach_10_sql = []byte("")

func migrations_sql_cockroach_10_sql() ([]byte, error) {
	return bindata_read(
		_migrations_sql_cockroach_10_sql,
		"migrations/sql/cockroach/10.sql",
	)
}

var _migrations_sql_cockroach_11_sql = []byte("")

func migrations_sql_cockroach_11_sql() ([]byte, error) {
	return bindata_read(
		_migrations_sql_cockroach_11_sql,
		"migrations/sql/cockroach/11.sql",
	)
}

var _migrations_sql_cockroach_9_sql = []byte("")

func migrations_sql_cockroach_9_sql() ([]byte, error) {
	return bindata_read(
		_migrations_sql_cockroach_9_sql,
		"migrations/sql/cockroach/9.sql",
	)
}

var _migrations_sql_mysql_gitkeep = []byte("\x1f\x8b\x08\x00\x00\x00\x00\x00\x00\xff\x01\x00\x00\xff\xff\x00\x00\x00\x00\x00\x00\x00\x00")

func migrations_sql_mysql_gitkeep() ([]byte, error) {
	return bindata_read(
		_migrations_sql_mysql_gitkeep,
		"migrations/sql/mysql/.gitkeep",
	)
}

var _migrations_sql_mysql_10_sql = []byte("")

func migrations_sql_mysql_10_sql() ([]byte, error) {
	return bindata_read(
		_migrations_sql_mysql_10_sql,
		"migrations/sql/mysql/10.sql",
	)
}

var _migrations_sql_mysql_5_sql = []byte("")

func migrations_sql_mysql_5_sql() ([]byte, error) {
	return bindata_read(
		_migrations_sql_mysql_5_sql,
		"migrations/sql/mysql/5.sql",
	)
}

var _migrations_sql_mysql_6_sql = []byte("")

func migrations_sql_mysql_6_sql() ([]byte, error) {
	return bindata_read(
		_migrations_sql_mysql_6_sql,
		"migrations/sql/mysql/6.sql",
	)
}

var _migrations_sql_mysql_7_sql = []byte("")

func migrations_sql_mysql_7_sql() ([]byte, error) {
	return bindata_read(
		_migrations_sql_mysql_7_sql,
		"migrations/sql/mysql/7.sql",
	)
}

var _migrations_sql_mysql_9_sql = []byte("")

func migrations_sql_mysql_9_sql() ([]byte, error) {
	return bindata_read(
		_migrations_sql_mysql_9_sql,
		"migrations/sql/mysql/9.sql",
	)
}

var _migrations_sql_postgres_gitkeep = []byte("\x1f\x8b\x08\x00\x00\x00\x00\x00\x00\xff\x01\x00\x00\xff\xff\x00\x00\x00\x00\x00\x00\x00\x00")

func migrations_sql_postgres_gitkeep() ([]byte, error) {
	return bindata_read(
		_migrations_sql_postgres_gitkeep,
		"migrations/sql/postgres/.gitkeep",
	)
}

var _migrations_sql_postgres_10_sql = []byte("")

func migrations_sql_postgres_10_sql() ([]byte, error) {
	return bindata_read(
		_migrations_sql_postgres_10_sql,
		"migrations/sql/postgres/10.sql",
	)
}

var _migrations_sql_postgres_5_sql = []byte("")

func migrations_sql_postgres_5_sql() ([]byte, error) {
	return bindata_read(
		_migrations_sql_postgres_5_sql,
		"migrations/sql/postgres/5.sql",
	)
}

var _migrations_sql_postgres_6_sql = []byte("")

func migrations_sql_postgres_6_sql() ([]byte, error) {
	return bindata_read(
		_migrations_sql_postgres_6_sql,
		"migrations/sql/postgres/6.sql",
	)
}

var _migrations_sql_postgres_7_sql = []byte("")

func migrations_sql_postgres_7_sql() ([]byte, error) {
	return bindata_read(
		_migrations_sql_postgres_7_sql,
		"migrations/sql/postgres/7.sql",
	)
}

var _migrations_sql_postgres_9_sql = []byte("")

func migrations_sql_postgres_9_sql() ([]byte, error) {
	return bindata_read(
		_migrations_sql_postgres_9_sql,
		"migrations/sql/postgres/9.sql",
	)
}

var _migrations_sql_shared_1_sql = []byte("")

func migrations_sql_shared_1_sql() ([]byte, error) {
	return bindata_read(
		_migrations_sql_shared_1_sql,
		"migrations/sql/shared/1.sql",
	)
}

var _migrations_sql_shared_11_sql = []byte("")

func migrations_sql_shared_11_sql() ([]byte, error) {
	return bindata_read(
		_migrations_sql_shared_11_sql,
		"migrations/sql/shared/11.sql",
	)
}

var _migrations_sql_shared_2_sql = []byte("")

func migrations_sql_shared_2_sql() ([]byte, error) {
	return bindata_read(
		_migrations_sql_shared_2_sql,
		"migrations/sql/shared/2.sql",
	)
}

var _migrations_sql_shared_3_sql = []byte("")

func migrations_sql_shared_3_sql() ([]byte, error) {
	return bindata_read(
		_migrations_sql_shared_3_sql,
		"migrations/sql/shared/3.sql",
	)
}

var _migrations_sql_shared_4_sql = []byte("")

func migrations_sql_shared_4_sql() ([]byte, error) {
	return bindata_read(
		_migrations_sql_shared_4_sql,
		"migrations/sql/shared/4.sql",
	)
}

var _migrations_sql_shared_8_sql = []byte("")

func migrations_sql_shared_8_sql() ([]byte, error) {
	return bindata_read(
		_migrations_sql_shared_8_sql,
		"migrations/sql/shared/8.sql",
	)
}

var _migrations_sql_tests_gitkeep = []byte("\x1f\x8b\x08\x00\x00\x00\x00\x00\x00\xff\x01\x00\x00\xff\xff\x00\x00\x00\x00\x00\x00\x00\x00")

func migrations_sql_tests_gitkeep() ([]byte, error) {
	return bindata_read(
		_migrations_sql_tests_gitkeep,
		"migrations/sql/tests/.gitkeep",
	)
}

var _migrations_sql_tests_10_test_sql = []byte("")

func migrations_sql_tests_10_test_sql() ([]byte, error) {
	return bindata_read(
		_migrations_sql_tests_10_test_sql,
		"migrations/sql/tests/10_test.sql",
	)
}

var _migrations_sql_tests_11_test_sql = []byte("")

func migrations_sql_tests_11_test_sql() ([]byte, error) {
	return bindata_read(
		_migrations_sql_tests_11_test_sql,
		"migrations/sql/tests/11_test.sql",
	)
}

var _migrations_sql_tests_1_test_sql = []byte("")

func migrations_sql_tests_1_test_sql() ([]byte, error) {
	return bindata_read(
		_migrations_sql_tests_1_test_sql,
		"migrations/sql/tests/1_test.sql",
	)
}

var _migrations_sql_tests_2_test_sql = []byte("")

func migrations_sql_tests_2_test_sql() ([]byte, error) {
	return bindata_read(
		_migrations_sql_tests_2_test_sql,
		"migrations/sql/tests/2_test.sql",
	)
}

var _migrations_sql_tests_3_test_sql = []byte("")

func migrations_sql_tests_3_test_sql() ([]byte, error) {
	return bindata_read(
		_migrations_sql_tests_3_test_sql,
		"migrations/sql/tests/3_test.sql",
	)
}

var _migrations_sql_tests_4_test_sql = []byte("")

func migrations_sql_tests_4_test_sql() ([]byte, error) {
	return bindata_read(
		_migrations_sql_tests_4_test_sql,
		"migrations/sql/tests/4_test.sql",
	)
}

var _migrations_sql_tests_5_test_sql = []byte("")

func migrations_sql_tests_5_test_sql() ([]byte, error) {
	return bindata_read(
		_migrations_sql_tests_5_test_sql,
		"migrations/sql/tests/5_test.sql",
	)
}

var _migrations_sql_tests_6_test_sql = []byte("")

func migrations_sql_tests_6_test_sql() ([]byte, error) {
	return bindata_read(
		_migrations_sql_tests_6_test_sql,
		"migrations/sql/tests/6_test.sql",
	)
}

var _migrations_sql_tests_7_test_sql = []byte("")

func migrations_sql_tests_7_test_sql() ([]byte, error) {
	return bindata_read(
		_migrations_sql_tests_7_test_sql,
		"migrations/sql/tests/7_test.sql",
	)
}

var _migrations_sql_tests_8_test_sql = []byte("")

func migrations_sql_tests_8_test_sql() ([]byte, error) {
	return bindata_read(
		_migrations_sql_tests_8_test_sql,
		"migrations/sql/tests/8_test.sql",
	)
}

var _migrations_sql_tests_9_test_sql = []byte("")

func migrations_sql_tests_9_test_sql() ([]byte, error) {
	return bindata_read(
		_migrations_sql_tests_9_test_sql,
		"migrations/sql/tests/9_test.sql",
	)
}

// Asset loads and returns the asset for the given name.
// It returns an error if the asset could not be found or
// could not be loaded.
func Asset(name string) ([]byte, error) {
	cannonicalName := strings.Replace(name, "\\", "/", -1)
	if f, ok := _bindata[cannonicalName]; ok {
		return f()
	}
	return nil, fmt.Errorf("Asset %s not found", name)
}

// AssetNames returns the names of the assets.
func AssetNames() []string {
	names := make([]string, 0, len(_bindata))
	for name := range _bindata {
		names = append(names, name)
	}
	return names
}

// _bindata is a table, holding each asset generator, mapped to its name.
var _bindata = map[string]func() ([]byte, error){
	"migrations/sql/cockroach/10.sql": migrations_sql_cockroach_10_sql,
	"migrations/sql/cockroach/11.sql": migrations_sql_cockroach_11_sql,
	"migrations/sql/cockroach/9.sql": migrations_sql_cockroach_9_sql,
	"migrations/sql/mysql/.gitkeep": migrations_sql_mysql_gitkeep,
	"migrations/sql/mysql/10.sql": migrations_sql_mysql_10_sql,
	"migrations/sql/mysql/5.sql": migrations_sql_mysql_5_sql,
	"migrations/sql/mysql/6.sql": migrations_sql_mysql_6_sql,
	"migrations/sql/mysql/7.sql": migrations_sql_mysql_7_sql,
	"migrations/sql/mysql/9.sql": migrations_sql_mysql_9_sql,
	"migrations/sql/postgres/.gitkeep": migrations_sql_postgres_gitkeep,
	"migrations/sql/postgres/10.sql": migrations_sql_postgres_10_sql,
	"migrations/sql/postgres/5.sql": migrations_sql_postgres_5_sql,
	"migrations/sql/postgres/6.sql": migrations_sql_postgres_6_sql,
	"migrations/sql/postgres/7.sql": migrations_sql_postgres_7_sql,
	"migrations/sql/postgres/9.sql": migrations_sql_postgres_9_sql,
	"migrations/sql/shared/1.sql": migrations_sql_shared_1_sql,
	"migrations/sql/shared/11.sql": migrations_sql_shared_11_sql,
	"migrations/sql/shared/2.sql": migrations_sql_shared_2_sql,
	"migrations/sql/shared/3.sql": migrations_sql_shared_3_sql,
	"migrations/sql/shared/4.sql": migrations_sql_shared_4_sql,
	"migrations/sql/shared/8.sql": migrations_sql_shared_8_sql,
	"migrations/sql/tests/.gitkeep": migrations_sql_tests_gitkeep,
	"migrations/sql/tests/10_test.sql": migrations_sql_tests_10_test_sql,
	"migrations/sql/tests/11_test.sql": migrations_sql_tests_11_test_sql,
	"migrations/sql/tests/1_test.sql": migrations_sql_tests_1_test_sql,
	"migrations/sql/tests/2_test.sql": migrations_sql_tests_2_test_sql,
	"migrations/sql/tests/3_test.sql": migrations_sql_tests_3_test_sql,
	"migrations/sql/tests/4_test.sql": migrations_sql_tests_4_test_sql,
	"migrations/sql/tests/5_test.sql": migrations_sql_tests_5_test_sql,
	"migrations/sql/tests/6_test.sql": migrations_sql_tests_6_test_sql,
	"migrations/sql/tests/7_test.sql": migrations_sql_tests_7_test_sql,
	"migrations/sql/tests/8_test.sql": migrations_sql_tests_8_test_sql,
	"migrations/sql/tests/9_test.sql": migrations_sql_tests_9_test_sql,
}
// AssetDir returns the file names below a certain
// directory embedded in the file by go-bindata.
// For example if you run go-bindata on data/... and data contains the
// following hierarchy:
//     data/
//       foo.txt
//       img/
//         a.png
//         b.png
// then AssetDir("data") would return []string{"foo.txt", "img"}
// AssetDir("data/img") would return []string{"a.png", "b.png"}
// AssetDir("foo.txt") and AssetDir("notexist") would return an error
// AssetDir("") will return []string{"data"}.
func AssetDir(name string) ([]string, error) {
	node := _bintree
	if len(name) != 0 {
		cannonicalName := strings.Replace(name, "\\", "/", -1)
		pathList := strings.Split(cannonicalName, "/")
		for _, p := range pathList {
			node = node.Children[p]
			if node == nil {
				return nil, fmt.Errorf("Asset %s not found", name)
			}
		}
	}
	if node.Func != nil {
		return nil, fmt.Errorf("Asset %s not found", name)
	}
	rv := make([]string, 0, len(node.Children))
	for name := range node.Children {
		rv = append(rv, name)
	}
	return rv, nil
}

type _bintree_t struct {
	Func func() ([]byte, error)
	Children map[string]*_bintree_t
}
var _bintree = &_bintree_t{nil, map[string]*_bintree_t{
	"migrations": &_bintree_t{nil, map[string]*_bintree_t{
		"sql": &_bintree_t{nil, map[string]*_bintree_t{
			"cockroach": &_bintree_t{nil, map[string]*_bintree_t{
				"10.sql": &_bintree_t{migrations_sql_cockroach_10_sql, map[string]*_bintree_t{
				}},
				"11.sql": &_bintree_t{migrations_sql_cockroach_11_sql, map[string]*_bintree_t{
				}},
				"9.sql": &_bintree_t{migrations_sql_cockroach_9_sql, map[string]*_bintree_t{
				}},
			}},
			"mysql": &_bintree_t{nil, map[string]*_bintree_t{
				".gitkeep": &_bintree_t{migrations_sql_mysql_gitkeep, map[string]*_bintree_t{
				}},
				"10.sql": &_bintree_t{migrations_sql_mysql_10_sql, map[string]*_bintree_t{
				}},
				"5.sql": &_bintree_t{migrations_sql_mysql_5_sql, map[string]*_bintree_t{
				}},
				"6.sql": &_bintree_t{migrations_sql_mysql_6_sql, map[string]*_bintree_t{
				}},
				"7.sql": &_bintree_t{migrations_sql_mysql_7_sql, map[string]*_bintree_t{
				}},
				"9.sql": &_bintree_t{migrations_sql_mysql_9_sql, map[string]*_bintree_t{
				}},
			}},
			"postgres": &_bintree_t{nil, map[string]*_bintree_t{
				".gitkeep": &_bintree_t{migrations_sql_postgres_gitkeep, map[string]*_bintree_t{
				}},
				"10.sql": &_bintree_t{migrations_sql_postgres_10_sql, map[string]*_bintree_t{
				}},
				"5.sql": &_bintree_t{migrations_sql_postgres_5_sql, map[string]*_bintree_t{
				}},
				"6.sql": &_bintree_t{migrations_sql_postgres_6_sql, map[string]*_bintree_t{
				}},
				"7.sql": &_bintree_t{migrations_sql_postgres_7_sql, map[string]*_bintree_t{
				}},
				"9.sql": &_bintree_t{migrations_sql_postgres_9_sql, map[string]*_bintree_t{
				}},
			}},
			"shared": &_bintree_t{nil, map[string]*_bintree_t{
				"1.sql": &_bintree_t{migrations_sql_shared_1_sql, map[string]*_bintree_t{
				}},
				"11.sql": &_bintree_t{migrations_sql_shared_11_sql, map[string]*_bintree_t{
				}},
				"2.sql": &_bintree_t{migrations_sql_shared_2_sql, map[string]*_bintree_t{
				}},
				"3.sql": &_bintree_t{migrations_sql_shared_3_sql, map[string]*_bintree_t{
				}},
				"4.sql": &_bintree_t{migrations_sql_shared_4_sql, map[string]*_bintree_t{
				}},
				"8.sql": &_bintree_t{migrations_sql_shared_8_sql, map[string]*_bintree_t{
				}},
			}},
			"tests": &_bintree_t{nil, map[string]*_bintree_t{
				".gitkeep": &_bintree_t{migrations_sql_tests_gitkeep, map[string]*_bintree_t{
				}},
				"10_test.sql": &_bintree_t{migrations_sql_tests_10_test_sql, map[string]*_bintree_t{
				}},
				"11_test.sql": &_bintree_t{migrations_sql_tests_11_test_sql, map[string]*_bintree_t{
				}},
				"1_test.sql": &_bintree_t{migrations_sql_tests_1_test_sql, map[string]*_bintree_t{
				}},
				"2_test.sql": &_bintree_t{migrations_sql_tests_2_test_sql, map[string]*_bintree_t{
				}},
				"3_test.sql": &_bintree_t{migrations_sql_tests_3_test_sql, map[string]*_bintree_t{
				}},
				"4_test.sql": &_bintree_t{migrations_sql_tests_4_test_sql, map[string]*_bintree_t{
				}},
				"5_test.sql": &_bintree_t{migrations_sql_tests_5_test_sql, map[string]*_bintree_t{
				}},
				"6_test.sql": &_bintree_t{migrations_sql_tests_6_test_sql, map[string]*_bintree_t{
				}},
				"7_test.sql": &_bintree_t{migrations_sql_tests_7_test_sql, map[string]*_bintree_t{
				}},
				"8_test.sql": &_bintree_t{migrations_sql_tests_8_test_sql, map[string]*_bintree_t{
				}},
				"9_test.sql": &_bintree_t{migrations_sql_tests_9_test_sql, map[string]*_bintree_t{
				}},
			}},
		}},
	}},
}}
